    ...
    constructor(selector, columns, options = {
        ...
        this._makeHead = function () {
            ...
            for (let column in columns) {
                ...
                let width = val.width ? `style="width:${val.width}"` : '';
                head += `<th class="dlp-text text-white" ${width}>${val.name}</th>`;
            }
...
window.ComponentPlane = class {
    ...
    make() {
        ...
        if (this.OPTIONS.gauze) {
            ...
            gauze.append(Plane);
            if(this.PARENT_DOM instanceof HTMLElement) gauze.style.position = 'absolute';
            Plane = gauze;
        }
        if (this.PARENT_DOM instanceof HTMLElement) {
...
window.ComponentCascadeLine = class {
    constructor(selector, select, url, options = {
        ...
        this._makeSelect = function (select) {
            ...
            for (let stack in this.dimensional_data) {
                ...
                data.forEach((v, k) => {
                    ...
                    let labelDom = this._insertLabelDom(v, k, stack);
                    if (v.nodes !== null) {
        ...
        this._insertLabelDom = function (data, index, stack) {
            let div = document.createElement('div');
            div.className = 'dlp dlp-text dlp-label';
            ...
            div.addEventListener('click', () => this._select(div, stack));
            div.addEventListener("contextmenu", (e) => {
                e.preventDefault();
                if (e.target instanceof HTMLElement) e.target.click();
                let settings = [];
                if (this.OPTIONS.detail) settings.push({
                    title: '详情', func: () => {
                        this._nodeDetail(div, data);
                    }
                });
                if (this.OPTIONS.insert) settings.push({
                    title: '新增', func: () => {
                        this._nodeInsert(div, data, stack);
                    }
                });
                if (this.OPTIONS.update) settings.push({
                    title: '修改', func: () => {
                        this._nodeUpdate(div, data);
                    }
                });
                if (this.OPTIONS.delete) settings.push({
                    title: '删除', func: () => {
                        this._nodeDelete(div, data, stack);
                    }
                });
            ...
            if (this.OPTIONS.movable) {
                let I = document.createElement('i');
                I.className = 'right';
                I.insertAdjacentHTML('afterbegin', _component.move);
                this._nodeMigrate(I);
                div.append(I);
            }
            return div;
        ...
        this._select = function (element, stack) {
            ...
            this.dimensional_data[stack].forEach((data, index) => {
                ...
                this._expand(data, currentStackDocuments[index], false);
            });
            this._expand(data, element, true);
            element.classList.remove('dlp-label-silence');
            this._selectToChildren(stack + 1, data.nodes);
            if (Array.isArray(data.parentNodes) && data.parentNodes.length > 0) {
                let parentNodes = data.parentNodes.slice(0);
                this._selectToParent(parentNodes);
            }
        };

        this._selectToParent = function (nodes) {
            let stack = nodes.length - 1;
            let node = nodes.pop();
            let parentNode = nodes[stack - 1];
            let currentStackDocuments = this.STACKS[stack].childNodes;
        ...
        this._expand = function (data, dom, open = true) {
        ...
        /*** context menu curd + ***/
        this._nodeDetail = function (dom, data) {
        ...
        this._nodeInsert = function (dom, data, stack) {
                ...
                .bindRequest('button[type="submit"]', 'click', {
                    ...
                    callback: (response) => {
                        ...
                        if (!Array.isArray(object.dimensional_data[nextStack])) {
                            ...
                            let stackDom = document.createElement('div');
                            stackDom.className = 'dot-cascade-stack dlp-scroll';
                            stackDom.append(object._insertLabelDom(object.dimensional_data[nextStack][0], 0, nextStack));
                            ...
                            object.STACKS[nextStack].append(
                                object._insertLabelDom(object.dimensional_data[nextStack][index], index, nextStack));
                            dom.insertAdjacentHTML('afterbegin', `<i class="left">${_component.caret_right_circle}</i>`);
                            ...
                            for (let index in object.dimensional_data[nextStack]) {
                                ...
                                if (object.dimensional_data[nextStack][index].key === lastKey) {
                                    ...
                                    D.parentNode.insertBefore(
                                        object._insertLabelDom(object.dimensional_data[nextStack][newIndex], newIndex, nextStack), D.nextSibling);
                                    break;
                                }
                            }
        ...
        this._nodeUpdate = function (dom, data) {
        ...
        this._nodeDelete = function (dom, data, stack) {
            let object = this;
            ...
            D.addEventListener('click', (() => {
                ...
                _component.request({
                    ...
                    callback: function (response) {
                        ...
                        if (response.code !== 0) return _component.alert(response.message, 3, null, object.DOM);
                        object._nodeDeleteExec(data, stack);
                        object.PLANE_DOM.getDom().remove();
                    }
                });
            }));
            ...
            P.append(D);
            this.PLANE_DOM = new ComponentPlane(P, {
                width: '300px',
                height: '65px',
                f: false
            }).setParentDom(this.DOM).setTitle(title);
        ...
        this._nodeDeleteExec = function (data, stack) {
            /*parent data delete*/
                ...
                .bindEvent('div.dlp-button', 'click', () => {
                    ...
                    _component.request({
                        ...
                        callback: function (response) {
                            ...
                            if (response.code !== 0) return _component.alert(response.message, 3, null, object.DOM);
                            object._nodeRootExec(dom, node_data);
                            object.PLANE_DOM.getDom().remove();
        ...
        this._nodeRootExec = function (node, node_data) {
            ...
            if (node_data.nodes !== null) this._resetChildrenDimensional(node_data, node_data.stack + 1, diffStack);
            node_data.stack = 0;
            ...
            I.addEventListener('click', (() => {
                ...
                this.PLANE_DOM.bindRequest('button[type="submit"]', 'click', {
                    ...
                    callback: (response) => {
                        ...
                        currentStackDocuments.append(object._insertLabelDom(object.dimensional_data[0][lastKey], lastKey, 0));
        ...
        /*** migrate + ***/
        this._nodeMigrate = function (dom) {
            let object = this;
            let aim = null;
                    ...
                    if (el.getAttribute('drag-zone') === 'true') {
                        if (e.type === 'dragover') {
                            ...
                            if (!object.OPTIONS.exchange) {
                                ...
                                if (Array.isArray(parents) && (parents.indexOf(parseInt(el.getAttribute('data-id'))) === -1)) {
            ...
            function fc(e) {
                ...
                e.target.removeEventListener('dragend', fc);
                document.removeEventListener('dragover', f);
                document.removeEventListener('dragleave', f);
                if (aim instanceof HTMLElement) {
                    object._nodeMigrateDialog(e.target, aim);
                }
            }

            dom.addEventListener('mousedown', () => {
                dom.parentNode.click();
                dom.parentNode.setAttribute('draggable', 'true');
                dom.parentNode.style.setProperty('border', '1px dashed #3c2d2d');
                document.addEventListener('dragover', f);
                document.addEventListener('dragleave', f);
                dom.parentNode.addEventListener('dragend', fc);
            });
            dom.addEventListener('mouseup', () => {
                dom.parentNode.style.removeProperty('border');
                document.removeEventListener('dragover', f);
                document.removeEventListener('dragleave', f);
                dom.parentNode.removeEventListener('dragend', fc);
                if (aim instanceof HTMLElement) {
                    aim.style.removeProperty('background');
                    aim = null;
                }
        ...
        this._nodeMigrateDialog = function (node, aim_node) {
            let event;
                ...
                .bindEvent('div.dlp-button', 'click', () => {
                    ...
                    _component.request({
                        ...
                        data: {
                            ...
                            response = JSON.parse(response);
                            if (response.code !== 0) return _component.alert(response.message, 3, null, object.DOM);
                            if (event === 'exchange') object._nodeExchangeExec(node, node_data, aim_node, aim_node_data);
                            if (event === 'migrate') object._nodeMigrateExec(node, node_data, aim_node, aim_node_data);
                            object.PLANE_DOM.getDom().remove();
                        }, error_callback: function () {
                            object.submit_block = false;
                        }
        ...
        this._nodeMigrateExec = function (node, node_data, aim_node, aim_node_data) {
            /*parent*/
            ...
            this._resetChildrenDimensional(node_data, node_data.stack + 1, diffStack);
            this.dimensional_data = this.dimensional_data.filter((d) => {
                if (d.length > 0) return true;
                return false;
            });
        ...
        this._nodeExchangeExec = function (node, node_data, aim_node, aim_node_data) {
            /*node*/
            ...
            this._resetChildrenParent(node_data);
        ...
        this._resetChildrenDimensional = function (node, stack, diffStack) {
            ...
            this.dimensional_data[stack].forEach((d, index) => {
                if (Array.isArray(nodes) && nodes.indexOf(d.key) !== -1) {
                    ...
                    this._resetChildrenDimensional(d, stack + 1, diffStack);
                }
            });
            indexes.reverse();
            indexes.forEach((d) => {
        ...
        this._search = function (search) {
            ...
            previous.addEventListener('click', ()=>this._searchPick(false));
            let next = document.createElement('span');
            next.className = 'dlp-btn';
            next.textContent = '▶';
            next.addEventListener('click', ()=>this._searchPick(true));
        ...
        this._searchPick = function (toward = true) {
            let pick = this.search_result[(this.search_result_pick - 1)];
            this.STACKS[pick.stack].childNodes[pick.index].classList.remove('dlp-label-active');
            if (toward) {
    ...
    make() {
        ...
        this._makeSelect(this.select_data);
        if (this.OPTIONS.root) this._rootNodeInsert();
