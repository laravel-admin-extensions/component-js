...
function componentDot(name, selected, options) {
    ...
    function tagCal(cdom, operate) {
        for (let element of inputs) {
            if (element.getAttribute('name') == `${name}[data]`) {
                dataDOM = element;
            }
            if (element.getAttribute('name') == `${name}[insert]`) {
                insertDOM = element;
            }
            if (element.getAttribute('name') == `${name}[delete]`) {
                deleteDOM = element;
            }
        }
        let id = cdom.getAttribute('data-id');
        let text = cdom.innerText;
        let dataHash = JSON.parse(dataDOM.value);
        let insertHash = JSON.parse(insertDOM.value);
        let deleteHash = JSON.parse(deleteDOM.value);
        if (operate == 'insert') {
            dataHash[id] = text;
            dataDOM.value = JSON.stringify(dataHash);
            if (!selected[id] && !insertHash[id]) {
                insertHash[id] = text;
                insertDOM.value = JSON.stringify(insertHash);
            }
            if (deleteHash[id]) {
                delete deleteHash[id];
                deleteDOM.value = JSON.stringify(deleteHash);
            }
            return;
        }
        if (operate == 'delete') {
            delete dataHash[id];
            dataDOM.value = JSON.stringify(dataHash);
            if (selected[id] && !deleteHash[id]) {
                deleteHash[id] = text;
                deleteDOM.value = JSON.stringify(deleteHash);
            }
            if (insertHash[id]) {
                delete insertHash[id];
                insertDOM.value = JSON.stringify(insertHash);
            }
        }
    }
    ...
    let html = `<div style="width: 100%;display: grid; grid-template-rows: 42px 140px;border: 1px solid #ccc;border-radius: 5px">
        ...
        ${options_dom}</div></div>
        <input name="${name}[data]" value='${selected_str}' type="hidden">
...
function componentLine(name, columns, data) {
    function selectTd(td, type, value, column,attributes) {
        switch (type) {
            case 'text':
    ...
    dom.insertAdjacentHTML('afterbegin', `<style>#${name} tbody::-webkit-scrollbar { width: 0 !important }
        #${name} th,#${name} td{width: 100px}</style>
